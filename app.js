new Vue({
	...
	methods: {
		startGame: function(){
			...
			this.logs.push('The Game Has Started!');
		},
		attack: function(){
			if(this.playerHealth > 0 & this.monsterHealth > 0){
				...
				this.logs.push('Player Damage: ' + playerDamage);
				this.logs.push('Monster Damage: ' + monsterDamage);
				this.winner();
			}
		},
		specialAttack: function(){
			if(this.playerHealth > 0 & this.monsterHealth > 0){
				...
				this.monsterHealth -= playerDamage;
				this.playerHealth -= monsterDamage;
				this.logs.push('Player Damage (Special Attack): ' + playerDamage);
				this.logs.push('Monster Damage: ' + monsterDamage);
				this.winner();
			}
		},
		healHealth: function(){
			if(this.playerHealth > 0 & this.monsterHealth > 0){
				if(this.playerHealth == 100){
					this.logs.push('Player Health Is Full - Healing Ability Has No Use');
				}
				else{
					...
					this.logs.push('Monster Damage: ' + monsterDamage);
					this.logs.push('Player Health Healed: ' + healingFactor);
				}
			}	
		},
		giveUp: function(){
		...
		winner: function(){
			if(this.playerHealth <= 0 & this.monsterHealth > 0){
				this.logs = [];
				this.logs.push('You Have Lost!');
			}
			else if(this.monsterHealth <= 0 & this.playerHealth > 0){
				this.logs = [];
				this.logs.push('You Have Won!');
			}
		}
	}
});